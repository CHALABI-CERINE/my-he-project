...
import { ENDPOINTS } from '../config'; // <--- On utilise ça maintenant
...
export default function Demo() {
  ...
  const startProcess = async () => {
    ...
    try {
      ...
      if(nums.length > 2000) addLog(`Note: Chiffrement partiel (${demoLimit} items) pour fluidité démo navigateur.`, "info");
      ...
      // --- CORRECTION ICI : UTILISATION DE ENDPOINTS ---
      ...
      for(let i=0; i<uploadLimit; i++) {
        await fetch(ENDPOINTS.UPLOAD_CHUNK, {
          ...
          body: JSON.stringify(encryptedChunks[i])
        });
      }
            ...
            <div className="box-content" style={{flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center'}}>
              ...
              {!fileInfo && (
                  <div style={{display:'flex', gap:'10px', marginTop:'10px', justifyContent:'center'}}>
                      <button onClick={() => generateBigData(50000)} disabled={isGenerating} style={btnGenStyle}>50k (Medium)</button>
                      <button onClick={() => generateBigData(500000)} disabled={isGenerating} style={{...btnGenStyle, color:'#fcd34d'}}>500k (Large)</button>
                      <button onClick={() => generateBigData(1000000)} disabled={isGenerating} style={{...btnGenStyle, color:'#ef4444'}}>1M (Big Data)</button>
                  </div>
              )}
          ...
          <div className={`box-step ${activeStep === 3 ? 'active' : ''}`} style={{flex: 1, display: 'flex', flexDirection: 'column'}}>
            ...
            <div className="box-content" style={{flex: 1, display: 'flex', flexDirection: 'row', gap: '1rem', alignItems: 'center', justifyContent: 'center'}}>
                <div style={{flex: 1, textAlign: 'center', borderRight: '1px solid #374151'}}>
                    ...
                    <div style={{fontSize: '2rem', fontWeight: '800', color: resultMean ? '#10b981' : '#374151'}}>
                        {resultMean || "--"}
                    </div>
                </div>
                <div style={{flex: 1, textAlign: 'center'}}>
                    <div style={{fontSize: '0.75rem', color: '#9ca3af', textTransform: 'uppercase', marginBottom: '5px'}}>SOMME TOTALE</div>
                    <div style={{fontSize: '2rem', fontWeight: '800', color: resultSum ? '#3b82f6' : '#374151'}}>
                        {resultSum || "--"}
                    </div>
                </div>
            </div>
            ...
            <div className="terminal-content">
              {logs.map((l, i) => (
                <div key={i} style={{marginBottom: '4px', borderLeft: `2px solid ${l.type === 'error' ? '#ef4444' : l.type === 'success' ? '#10b981' : '#374151'}`, paddingLeft: '8px'}}>
                  ...
                  <span style={{color: l.type === 'error' ? '#ef4444' : l.type === 'warning' ? '#f59e0b' : l.type === 'success' ? '#10b981' : '#d1d5db'}}>
                    {l.msg}
                  </span>
                </div>
              ))}
